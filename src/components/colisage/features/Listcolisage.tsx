...
function ListColisage({setEstAjt,setShowColis,setModifier,setColis,ClientJson}:ListUtilisateurProps) {
  ...
  // }])
    ...
    const deleteProduct = (colis:Colis) => {
      axios.delete('http://localhost:1000/api/v1/colisage/deleteColis/'+colis.id)
      refetchColis()
    };
    ...
    <Section>
            ...
            >
                  ...
                  archive.current(Ville.id);
                                ...
                                edit={()=>FromDetails(p)}
                                obj={p}
                                update={()=>FormAsUpdate(p)}
                                /></Table.td>
                            </Table.tr>
                        )
                    })}
            </Table>
            <Pagin load={loadPage} visible={colisJson?.length ? true : false} max={colisJson?.length}  />
