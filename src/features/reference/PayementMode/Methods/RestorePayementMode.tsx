...
import { useRestoreClientMutation, useRestorePayementModeMutation } from "config/rtk";
...
const RestorePayementMode = ({ id }: RestorePayementModePorp, ref: Ref<void>) => {
  ...
  const [restore] = useRestorePayementModeMutation();
  ...
  const restoreTemp = () => {
    axios
      .patch("https://gescom-api.frimakers.com/api/v1/payementModes/" + id0 + "/restore")
      .then(() => { });
  };
  return (
    <>
...
export default forwardRef(RestorePayementMode);
