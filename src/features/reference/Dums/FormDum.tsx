...
import { BUREAU, DECLARANTS, FOURNISSEUR, REGIME, REQUEST_EDIT, REQUEST_SAVE } from "tools/consts";
...
const FormDum = ({
        ...
        return ([
            {
                ...
                text: "Détail",
                action: () => { open(dum); setRequest(REQUEST_EDIT); setDisabled(true) },
            },
            {
                icon: (
                    <PencilAltIcon
                ...
                text: "Modifier",
                action: () => { open(dum); setRequest(REQUEST_EDIT); setDisabled(false) },
            },
            {
                icon: (
                    <TrashIcon
    ...
    return (
        ...
        <>
            {!form && (
                <section className='bg-white float-left w-full h-full mp-8 shadow-lg'>
                    ...
                    <Table className='tab-list float-left w-full mt-8 tab-list float-left w-full'
                        thead={
                            <tr>
                                <th className=' top-0 z-10    py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900'>N° Dum</th>
                                <th className=' top-0 z-10    py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900'>Fournisseur</th>
                                <th className=' top-0 z-10    py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 '>Déclarant</th>
                                <th className=' top-0 z-10    py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900'>Bureau</th>
                                <th className=' top-0 z-10    py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900'>Régime</th>
                                <th className=' top-0 z-10    py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900'>Valeur</th>
                            ...
                            dums?.map((dum: Dum) => {
                                return (
                                    ...
                                    <tr key={dum.id}>
                                        ...
                                        <Table.td>{dum.valeur}{"$"}</Table.td>
                                        <Table.td>{dum.date}</Table.td>

                                        <Table.td className='cursor-pointer'><Mitems menu={menu(dum)} /></Table.td>
                                    </tr>
                                )
                            })
                        }
            ...
            <Modal show={show} title="Nouvelle Dum" format={+classNames("5")} close={closed}>
                <div className="float-left w-full">
                    <Form defaultValues={dum1} onSubmit={request == REQUEST_SAVE ? saveDum : request == REQUEST_EDIT ? editDum : void_}>
                        <div className="float-left w-full">
                            <div>
                            ...
                            <div>
                                <div className="float-left w-1/2">
                                    <Field label="Date" name="date" type="date" disabled={disabled} />
                                </div>
                                <div className="float-left w-1/2">
                                    <Field
                                        label="Fournisseur"
