...
import { CLIENTS, DECLARANTS, REQUEST_EDIT, REQUEST_SAVE, TRANSPORTEURS } from "tools/consts";
...
const FormDecharge = ({
    decharge,
}: FormDechargeProps, ref: Ref<void>) => {
        ...
        return ([
            {
                ...
                text: "Détail",
                action: () => { open(decharge); setRequest(REQUEST_EDIT); setDisabled(true) },
            },
            {
                icon: (
                    <PencilAltIcon
                ...
                text: "Modifier",
                action: () => { open(decharge); setRequest(REQUEST_EDIT); setDisabled(false) },
            },
            {
                icon: (
                    <TrashIcon
    ...
    return (
        ...
        <>
                    ...
                    <Table className='tab-list float-left w-full mt-8 tab-list float-left w-full'
                        thead={
                            <tr>
                                ...
                                <th className=' top-0 z-10    py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900'>Transporteur</th>
                            ...
                            decharges?.map((decharge: Decharge) => {
                                return (
                                    ...
                                    <tr key={decharge.id}>
                                        ...
                                        <Table.td>{decharge.valeur}{"$"}</Table.td>
                                        <Table.td>{decharge.date}</Table.td>

                                        <Table.td className='cursor-pointer'><Mitems menu={menu(decharge)} /></Table.td>
                                    </tr>
                                )
                            })
                        }
            ...
            <Modal show={show} title="Nouvelle Décharge" format={+classNames("5")} close={closed}>
                <div className="float-left w-full">
                    <Form defaultValues={decharge1} onSubmit={request == REQUEST_SAVE ? saveDecharge : request == REQUEST_EDIT ? editDecharge : void_}>
                        <div className="float-left w-full">
                            <div>
