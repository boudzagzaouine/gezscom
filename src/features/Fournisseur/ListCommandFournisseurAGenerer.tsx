...
import { useFetchCommandesFournisseurQuery, usePaginationCommandesFournisseurQuery } from 'config/rtk';
...
const ListCommandFournisseurAGenerer = () => {
    ...
    const { data = [], isFetching, refetch } = useFetchCommandesFournisseurQuery();
  return (
    <Section>
          <div className="float-left w-full">
          <h1 className="float-left"><b> Commandes Fournisseurs Générée non validées</b></h1>
          ...
          <tr>
                  ...
                  <Table.th>
                    <th className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 md:pr-0">
                      <Button className="inline-flex items-center px-2 py2 border border-transparent text-xs font-medium rounded-full shadow-sm text-white bg-fuchsia-600 hover:bg-fuchsia-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Valider</Button>
                    </th>
                  </Table.th>
          </tr>
        }
      >
                  ...
                  data.content?.map((commande) => (
                    <tr key={commande.id}>
                      ...
                      <Table.td><td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-semibold sm:pr-6 md:pr-0">
                      <Button className="text-lime-500 hover:text-lime-800">Modifier<span className="sr-only">, Lindsay Walton</span></Button>
                      </td></Table.td>
                      <Table.td><td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-semibold sm:pr-6 md:pr-0">
                      <Button className="text-fuchsia-600 hover:text-fuchsia-900">Valider<span className="sr-only">, Lindsay Walton</span></Button>
                      </td></Table.td>
                    </tr>
                  ))
                }
