...
import { ArchiveIcon, ReplyIcon, TrashIcon, XIcon } from "@heroicons/react/solid";
...
const Box = ({ children, title, show, action, close }: BoxProps) => {
  ...
  const act=action==ARCHIVE?"archive":action==RESTORE?"restore":action==DEL?"del":""
  const style="float-left h-8 w-8 text-rose-900"
  const icon=action==ARCHIVE?<ArchiveIcon className={style} aria-hidden="true" />:action==RESTORE?<ReplyIcon className={style} aria-hidden="true" />:action==DEL?<TrashIcon className={style} aria-hidden="true" />:<></>
  const open = () => {
    //setShowBox(true);
  };
  useEffect(() => {
    ...
    <>
      {showBox && (
        <>
          ...
          <div className="mt ml-2 text-lg float-left text-slate-50">Demande de confirmation</div>          
          </div>
          <button
      className={"absolute top-0 right-0 cursor-pointer px-2.5 py bg-transparent"}
      onClick={() => {
        close();
      }}
    >
